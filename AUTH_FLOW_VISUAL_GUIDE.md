# 🎯 Authentication Flow - Visual Guide

## 📱 Complete User Journey Map

```
┌─────────────────────────────────────────────────────────────────────────┐
│                          🏠 APP LAUNCH                                  │
└──────────────────────┬──────────────────────────────────────────────────┘
                       │
                       ▼
              ┌────────────────┐
              │  Auth Screen   │
              │  app/auth.tsx  │
              └────────┬───────┘
                       │
         ┌─────────────┴─────────────┐
         │                           │
         ▼                           ▼
    ┌──────────┐              ┌──────────┐
    │ Sign Up  │              │ Sign In  │
    │   Tab    │              │   Tab    │
    └────┬─────┘              └────┬─────┘
         │                         │
         │ Enter email +           │ Enter email +
         │ password                │ password
         ▼                         ▼
    ┌──────────────────┐      ┌──────────────────┐
    │ Tap "Create      │      │ Tap "Sign In"    │
    │  Account"        │      │                  │
    └────┬─────────────┘      └────┬─────────────┘
         │                         │
         │ API Call to             │ API Call to
         │ Supabase                │ Supabase
         ▼                         ▼
    ┌──────────────────┐      ┌──────────────────┐
    │ Success?         │      │ Email verified?  │
    └────┬─────────────┘      └────┬─────────────┘
         │                         │
    ┌────┴────┐                   ┌┴────┐
    │         │                   │     │
    ▼         ▼                   ▼     ▼
  ✅ YES    ❌ ERROR            ✅ YES  ❌ NO
    │         │                   │     │
    │         └──────┐            │     └────────┐
    │                │            │              │
    ▼                ▼            ▼              ▼
┌────────────┐  ┌──────────┐  ┌──────┐    ┌──────────┐
│ Navigate   │  │ Show     │  │ Go   │    │ Show     │
│ to Email   │  │ Error    │  │ Home │    │ "Please  │
│ Verif.     │  │ Alert    │  │      │    │ verify"  │
│ Screen     │  │          │  │      │    │ message  │
└─────┬──────┘  └──────────┘  └──────┘    └──────────┘
      │
      ▼
┌──────────────────────────────────────────────┐
│    📧 Email Verification Screen              │
│    app/email-verification.tsx                │
│                                              │
│  ✉️  Verify Your Email                      │
│                                              │
│  We've sent a link to: user@example.com     │
│                                              │
│  Next Steps:                                │
│  1️⃣ Check your inbox                        │
│  2️⃣ Click verification link                 │
│  3️⃣ Return here                             │
│                                              │
│  [Resend Email]  [← Back to Sign In]        │
└──────┬───────────────────────────────────────┘
       │
       │ User opens email
       │ on their device
       ▼
┌──────────────────────────────────────────────┐
│         📧 Email App (Gmail, etc.)           │
│                                              │
│  Subject: Confirm Your Email                │
│                                              │
│  Please confirm your email:                 │
│  [Confirm your email] ← Click here          │
│                                              │
└──────┬───────────────────────────────────────┘
       │
       │ User taps link
       │
       ▼
┌──────────────────────────────────────────────┐
│         🔗 Deep Link Triggered               │
│         asrariya://auth/callback?...            │
│                                              │
│  Operating System intercepts URL            │
│  Checks: Is "asrariya://" registered?          │
│  ✅ YES → Open Asrar app                     │
└──────┬───────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────┐
│    📱 App Opens / Resumes                    │
│    DeepLinkHandler in app/_layout.tsx       │
│                                              │
│  1. Parse URL parameters                    │
│  2. Extract access_token, refresh_token     │
│  3. Check for errors                        │
└──────┬───────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────┐
│         🔍 Profile Check                     │
│                                              │
│  Does user have profile data?               │
│  (nameAr, nameLatin, or dobISO)             │
└──────┬───────────────────────────────────────┘
       │
   ┌───┴───┐
   │       │
   ▼       ▼
  ✅ YES  ❌ NO
   │       │
   │       └─────────────────────┐
   │                             │
   ▼                             ▼
┌──────────────┐          ┌──────────────────┐
│ Show Alert:  │          │ Show Alert:      │
│ "Welcome!"   │          │ "Complete your   │
│              │          │  profile"        │
│ Navigate to  │          │                  │
│ Home Screen  │          │ Navigate to      │
│              │          │ Profile Screen   │
└──────────────┘          └─────┬────────────┘
                                │
                                ▼
                    ┌─────────────────────┐
                    │  📋 Profile Screen  │
                    │  app/profile.tsx    │
                    │                     │
                    │ User enters:        │
                    │ - Name              │
                    │ - Date of Birth     │
                    │ - Mother's name     │
                    │ - Location          │
                    │                     │
                    │ [Save]              │
                    └────┬────────────────┘
                         │
                         ▼
                    ┌─────────────┐
                    │ Navigate to │
                    │ Home Screen │
                    └─────────────┘
```

---

## 🔄 Error Handling Flow

```
Sign Up Attempt
      │
      ▼
┌─────────────────┐
│ Validation      │
│ - Email format  │
│ - Password len  │
│ - Password match│
└────┬────────────┘
     │
     ├──❌ Invalid → Show inline error
     │
     ├──✅ Valid → Call API
     │
     ▼
┌─────────────────────┐
│ Supabase Response   │
└────┬────────────────┘
     │
     ├──✅ Success (EMAIL_CONFIRMATION_REQUIRED)
     │   └─→ Navigate to verification screen
     │
     ├──❌ Email exists (User already registered)
     │   └─→ "This email is already registered. Try signing in instead."
     │
     ├──❌ Weak password
     │   └─→ "Password must be at least 6 characters."
     │
     ├──❌ Network error
     │   └─→ "No internet connection. Please check your network."
     │
     └──❌ Other errors
         └─→ "Something went wrong. Please try again."
```

---

## 📡 Deep Link Processing Flow

```
Deep Link Received: asrariya://auth/callback?type=signup&access_token=xxx&...
      │
      ▼
┌──────────────────────┐
│ Parse URL            │
│ - Extract path       │
│ - Extract params     │
└────┬─────────────────┘
     │
     ▼
┌──────────────────────┐
│ Is path              │
│ "auth/callback"?     │
└────┬─────────────────┘
     │
     ├──❌ NO → Ignore
     │
     └──✅ YES
         │
         ▼
    ┌────────────────┐
    │ Check for      │
    │ error param    │
    └────┬───────────┘
         │
         ├──✅ Error exists
         │   └─→ Show alert
         │       Navigate to /auth
         │
         └──❌ No error
             │
             ▼
        ┌────────────────┐
        │ Verify params: │
        │ - type=signup  │
        │ - access_token │
        │ - refresh_token│
        └────┬───────────┘
             │
             ├──❌ Missing → Navigate to /auth
             │
             └──✅ All present
                 │
                 ▼
            ┌────────────────┐
            │ Update profile │
            │ mode='account' │
            └────┬───────────┘
                 │
                 ▼
            ┌────────────────┐
            │ Check profile  │
            │ data exists?   │
            └────┬───────────┘
                 │
            ┌────┴────┐
            │         │
            ▼         ▼
          YES        NO
            │         │
            │         └─→ Navigate to /profile
            │
            └─→ Navigate to /(tabs)
```

---

## 🎨 Screen-by-Screen Breakdown

### 1️⃣ Auth Screen (`app/auth.tsx`)
```
┌────────────────────────────────┐
│         Asrār ✦                │
│  Sacred Numerology & Mysticism │
├────────────────────────────────┤
│  [Sign Up] [Sign In]           │ ← Tab switcher
├────────────────────────────────┤
│  📧 Email                       │
│  [your@email.com________]      │
│                                │
│  🔒 Password                   │
│  [••••••••______________]      │
│                                │
│  🔒 Confirm Password           │ ← Only on Sign Up
│  [••••••••______________]      │
│                                │
│  [Create Account / Sign In]    │ ← Shows loading state
│                                │
│      or                        │
│                                │
│  [👤 Continue as Guest]        │
│                                │
│  🛡️ Your data is encrypted     │
└────────────────────────────────┘
```

### 2️⃣ Email Verification Screen (`app/email-verification.tsx`)
```
┌────────────────────────────────┐
│         ✉️                     │ ← Large icon
│                                │
│   Verify Your Email            │
│                                │
│ We've sent a verification      │
│ link to:                       │
│ user@example.com               │ ← User's email
│                                │
├────────────────────────────────┤
│ Next Steps:                    │
│ 1️⃣ Check your inbox            │
│ 2️⃣ Click verification link     │
│ 3️⃣ Return here                 │
├────────────────────────────────┤
│                                │
│  [📧 Resend Email]             │ ← With cooldown
│  or "Resend in 45s"            │
│                                │
│  ← Back to Sign In             │
│                                │
│ 🔒 Your data is encrypted      │
└────────────────────────────────┘
```

### 3️⃣ Profile Screen (`app/profile.tsx`) - Existing
```
┌────────────────────────────────┐
│      User Profile              │
├────────────────────────────────┤
│  Name (Arabic)                 │
│  [___________________]         │
│                                │
│  Name (Latin)                  │
│  [___________________]         │
│                                │
│  Date of Birth                 │
│  [📅 Select date_______]       │
│                                │
│  Mother's Name (optional)      │
│  [___________________]         │
│                                │
│  Location (optional)           │
│  [📍 Get location_____]        │
│                                │
│  [💾 Save Profile]             │
└────────────────────────────────┘
```

---

## 🔐 Security & Privacy Flow

```
User Data Journey:

1. Sign Up
   └─→ Password hashed by Supabase
       └─→ Never stored in plain text

2. Email Verification
   └─→ Token in URL (one-time use)
       └─→ Expires after 24 hours

3. Deep Link
   └─→ Access token & refresh token
       └─→ Stored in expo-secure-store
           └─→ Encrypted on device

4. Profile Data
   └─→ Stored locally
       └─→ Optionally synced to Supabase
           └─→ Row-level security enabled
               └─→ Only user can access own data
```

---

## 📊 State Management

```
ProfileContext (Global State)
      │
      ├─→ profile.mode
      │   ├─ 'guest' → Using app without account
      │   └─ 'account' → Signed in with email
      │
      ├─→ profile.nameAr
      ├─→ profile.nameLatin
      ├─→ profile.dobISO
      ├─→ profile.motherName
      ├─→ profile.location
      │
      └─→ completionStatus
          ├─ hasName: boolean
          ├─ hasDOB: boolean
          ├─ hasMother: boolean
          └─ hasLocation: boolean
```

---

*Visual guide for AUTH_FLOW_IMPLEMENTATION_COMPLETE.md*
